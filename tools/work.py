...
# `check` and `ci` run it whenever the lock exists (harness_gate), and strays()
# names each such file an order changed without owning it while it drifts.
...
def harness_gate(root: Path) -> list[str]:
    """Drift in this checkout's rendered files, whenever it has a lock: the
    check that frees them from owns has to run wherever owns is enforced."""
    if not (root / HARNESS_LOCK).is_file():
        return []
    try:
        return harness_drift(root)
    except Bad as e:
        return [str(e)]
...
def run_check(
    ...
    result["drift"] = harness_gate(order.root)
    ...
    result["ok"] = (
        ...
        and all(r.get("exit", 0) == 0 for r in rows)
    )
    if budget is None:
...
def show(result: dict) -> str:
    ...
    for d in result.get("drift", []):
        lines.append(f"  DRIFT  {d}  (harness.py check; run {HARNESS} sync)")
...
def cmd_ci(a: argparse.Namespace) -> int:
    ...
    bad = [f"harness.py check: {d}" for d in harness_gate(ROOT)]
