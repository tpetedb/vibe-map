...
function renderGalaxyList(){const ui=$("galaxy-ui");if(!ui)return;ui.hidden=false;
  ...
  $("galaxy-list").innerHTML=model.places.map(place=>`<div class="galaxy-place" role="listitem" data-state="${place.state}"><b>${esc(place.name)}</b><span class="galaxy-state">${place.state} · ${place.done} of ${place.count}</span><button data-galaxy-place="${place.id}" onclick="galaxyPlace('${place.id}')">Show</button></div>`).join("");
...
EXPERIENCES.galaxy={
  ...
  build(){const model=galaxyModel(PLACES,TREE,galaxyDone()),groups=galaxyGroups(model),visuals=createGalaxyVisuals({three:T,palette:PALETTE,material:mat,finish:fixColors});
    scene=new T.Scene();scene.background=new T.Color(PALETTE.black);scene.add(new T.AmbientLight(PALETTE.snow,.55));
    const light=new T.DirectionalLight(PALETTE.snow,1.5);light.position.set(3,8,8);scene.add(light);
    ...
    const journey=visuals.makeJourney(nodes);scene.add(journey);galaxyState={model,groups,visuals,planets,journey,detail:null,selected:(model.next||model.places[0]||{}).place||(model.places[0]||{}).id,view:"journey"};
  ...
  listing(){return galaxyModel(PLACES,TREE,galaxyDone()).places.map(place=>({id:place.id,title:place.name,state:place.state,count:place.count,done:place.done}))},
};
