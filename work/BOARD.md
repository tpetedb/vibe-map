...
- **Codex.** Trust the project. Start every fresh or resumed session with `just codex` (for example `just codex resume --last`), which runs `codex "$@" --add-dir "$(git rev-parse --path-format=absolute --git-common-dir)/board"`: the tracked `.codex/config.toml` names no writable root, because a relative one is not a directory in a linked worktree and Codex then refuses every shell tool, so a plain `codex` session reads the board and `board.py` tells it this flag when it writes; nothing under `.agents/` or elsewhere in `.git` is writable. Codex asks you to trust the SessionStart hook in its own `/hooks` prompt; grant it there and bypass nothing. No MCP server is registered on either client.
