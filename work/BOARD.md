...
- **A new worktree needs `just codex-trust` once.** Codex reads project trust only from `[projects."<absolute path>"]` in `~/.codex/config.toml`. `just codex-trust [path]` (default: the current checkout) adds that table with `trust_level = "trusted"` when it is absent, for a checkout whose git common dir is this repository's, and refuses any other path; it never touches hook trust and never writes a hash. The worktree's project hooks stay off until someone approves them in `/hooks` there, in Codex's own prompt, bypassing nothing; until then `just codex` puts the output of `tools/board.py read` in front of the first prompt, so the digest comes by instruction. No MCP server is registered on either client.
