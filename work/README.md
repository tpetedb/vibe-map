...
- In Claude Code, a hook refuses an edit outside the orders on the current branch. It lets go on anything it cannot read, so a broken or half-written order stays repairable; `work-check` refuses that order later anyway. The edit hook also notes which agent worked on which order (`touched.json`, not committed), and an agent that worked on one cannot stop while it fails: a builder is held to the checks, someone who only wrote into the order's folder to a readable review. A report whose first line is `order: <id>` counts the same way. The stop hook blocks once and lets go on the second try.
