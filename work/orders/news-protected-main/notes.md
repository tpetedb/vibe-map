...
An existing open PR is left for review. Any existing branch without an open PR
is preserved and reported as requiring attention, including a merged branch that
was not deleted. After merging, the maintainer must confirm the branch's work is
integrated and delete it before the next update. This deliberately preserves
