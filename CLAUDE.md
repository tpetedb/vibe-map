...
- Work orders (`work/`, skill `work-order`): subagents `builder` (Opus), `reviewer` and `team-manager` (Fable) exist in `.claude/agents/`. A PreToolUse hook refuses an edit outside the order on the current branch; a Stop and SubagentStop hook holds an agent that edited under an order until `just work-check <id>` passes (once; the second try is let through). At most four build at once.
