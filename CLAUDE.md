...
- Work orders (`work/`, skill `work-order`): subagents `builder`, `reviewer` and `team-manager` exist (all on Fable: mixing in Opus cost more overall than Fable alone) in `.claude/agents/`. A PreToolUse hook refuses an edit outside the order on the current branch; a SubagentStop hook sends a subagent that edited under an order back once if `just work-check <id>` fails, and a Stop hook does the same for a session whose report starts with `order: <id>`. They are reminders; `work-check`, `work-accept` and CI decide. At most four build at once.
