...
function setPose(c,p,seat){if(!c)return;c.pose=p;c.poseT=0;c.seatH=seat===undefined?c.seatH:seat;
  if(p!=="sit"){c.poseY=0;if(c.lap)c.lap.visible=false}}
  ...
  if(packTab==="inventory"){
    body=`<p class="small muted">${got.length} of ${ITEMS.items.length} picked up. Walk over one to collect it; each explains one idea and opens its topic in the tech tree.</p>`+
      Object.keys(WORLDS).map(w=>{const list=ITEMS.items.filter(i=>i.world===w);
        return `<div class="card"><h3>${WORLDS[w].name}</h3><p class="small muted">${list.filter(i=>got.includes(i.id)).length} of ${list.length}</p>`+
          list.map(i=>got.includes(i.id)
            ?`<div class="pathrow"><span><b>${i.name}</b><br><span class="muted small">${i.concept}</span></span><button onclick="openTopic('${i.topic}')" style="padding:4px 10px;font-size:12px">Topic</button></div>`
            :`<div class="pathrow"><span class="muted">Not found yet</span><span class="st">${i.kind}</span></div>`).join("")+`</div>`}).join("")}
...
let wantSit=false,avatarWired=false;
