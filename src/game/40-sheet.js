...
<p class="small muted">${esc(w?w.name:"")} · stop ${n} of ${stopCount()}</p>
<b class="cont-t">${esc(c.h)}, ${esc(c.n)}</b>
<p class="small">${esc(c.d)}</p>
<pre class="cmd">${esc(stopCommand(n))}</pre>
<div class="row"><button class="primary" onclick="${where==="title"?"continueHere":"openCh"}(${n})">Open it</button></div></div>`}
...
function renderMap(){const ok=S.done.length>=stopCount();const ev=CAMPAIGN[S.world||"campus"];const nx=nextStop();
  ...
  const here=`<p class="here">${icon("compass")}You are here: <b>${esc((WORLDS[S.world||"campus"]||{}).name||"")}</b> · ${S.done.filter(n=>n<=stopCount()).length} of ${stopCount()} delivered</p>`;
