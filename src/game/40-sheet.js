...
window.openSheet=function(id){closeVault();document.querySelectorAll("#sheet .screen").forEach(s=>s.classList.remove("on"));$(id).classList.add("on");$("sheet").classList.add("on");if(id==="s-map")renderMap();setTimeout(()=>$("sheet").scrollIntoView({behavior:"smooth",block:"start"}),30)};
...
window.choosePath=function(id,v){S.path[id]=v;save();if(props.mentors){const c=props.mentors.find(x=>x.id===id);if(c)c.ring.material.color.set(v==="deep"?"#22D3EE":"#FB7185")}if(v==="deep"){$("deep").classList.add("on");$("deep").scrollIntoView({behavior:"smooth",block:"nearest"})}else closeSheet();hud()};
...
function renderMap(){const ok=S.done.length===8;const ev=CAMPAIGN[S.world];
  $("plotlist").innerHTML=`<div class="evening">${ev.title}</div><p class="small muted">${ev.blurb}</p>`+CH.map((c,i)=>{const k=i+1,done=S.done.includes(k),locked=k>1&&!S.done.includes(k-1);
  ...
  return;
  $("plotlist").innerHTML=`<button class="date" onclick="openCh(0)">Before the evening, Pre-flight<br><span class="small" style="opacity:.75">Install Claude Code, git, Obsidian. 20 minutes, alone.</span></button>`+CH.map((c,i)=>{const k=i+1,done=S.done.includes(k),locked=k>1&&!S.done.includes(k-1);
    return `<button class="date${done?' pick':''}" ${locked?'disabled':''} onclick="openCh(${k})">${c.h}, ${c.n}${done?" (delivered)":locked?" (blocked by dependency)":""}<br><span class="small" style="opacity:.75">${c.d}</span></button>`}).join("")+
    `<button class="date" ${ok?'':'disabled'} onclick="openCh(9)">Calendar alignment${ok?"":" (pending eight OKRs)"}</button>`}
