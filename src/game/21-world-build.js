...
function buildWorld(id,carry){
  ...
  if(id==="winter"){const sl=new T.Group();sl.add(box(1.2,.15,.6,"#B44A46",0,.15,0));[-.25,.25].forEach(z=>sl.add(box(1.4,.06,.08,"#7B5128",0,.05,z)));sl.position.set(W.lake[0]+1,0,W.lake[1]-.5);mergeStatic(sl);scene.add(sl);props.sled=sl}
  // path
  ...
  if(W.bridge){const br=new T.Group();for(let i=0;i<7;i++)br.add(box(1.6,.14,.45,"#D9A76A",0,.35,-1.5+i*.5));br.add(box(.1,.9,3.6,"#7B5128",-.75,.7,0));br.add(box(.1,.9,3.6,"#7B5128",.75,.7,0));[-1.5,0,1.5].forEach(z=>{br.add(box(.12,.9,.12,"#7B5128",-.75,.55,z));br.add(box(.12,.9,.12,"#7B5128",.75,.55,z))});const tg=curve.getTangentAt(W.bridge[2]);br.position.set(W.bridge[0],0,W.bridge[1]);br.rotation.y=-Math.atan2(tg.z,tg.x)+Math.PI;mergeStatic(br);scene.add(br)}
  ...
  if(ex("stall")){const stall=new T.Group();stall.add(box(2,.9,1,"#A9743F",0,.45,0));stall.add(box(2.4,.1,1.4,"#E11D48",0,2,0));[-1,1].forEach(x=>stall.add(cyl(.05,.05,2,"#7B5128",x,1,.5,4)));[["#FFBF00",-.6],["#22C55E",0],["#F87171",.6]].forEach(([c,x])=>stall.add(sph(.22,c,x,1.05,.2,6)));const sp=P(4.5,2);stall.position.set(sp[0],0,sp[1]);stall.rotation.y=-.6;mergeStatic(stall);scene.add(stall);obstacles.push([sp[0],sp[1],1.3]);blobAdd(sp[0],sp[1],1.5)}
  if(ex("well")){const well=new T.Group();well.add(cyl(.7,.75,.8,"#9CA3AF",0,.4,0,8));well.add(cone(.9,.6,"#7B5128",0,1.9,0,6));[-.5,.5].forEach(x=>well.add(box(.1,1.4,.1,"#7B5128",x,1,0)));const wp=P(-3,-4);well.position.set(wp[0],0,wp[1]);mergeStatic(well);scene.add(well);obstacles.push([wp[0],wp[1],.9]);blobAdd(wp[0],wp[1],1.1)}
  if(ex("mountain")){const mt=new T.Group();mt.add(cone(6,7,id==="winter"?"#B8C4CF":"#8B7355",0,3.5,0,7));mt.add(cone(2.3,2.6,"#F8FAFC",0,5.7,0,7));const mp=P(-9,-14);mt.position.set(mp[0],0,mp[1]);mt.traverse(o=>{if(o.isMesh)o.castShadow=true});mergeStatic(mt);scene.add(mt);obstacles.push([mp[0],mp[1],4.5])}
  ...
  if(ex("harbour")){const dock=new T.Group();for(let i=0;i<9;i++)dock.add(box(.5,.12,2,"#D9A76A",i*.55,.1,0));[0,2.2,4.4].forEach(x=>{dock.add(cyl(.1,.12,1.4,"#7B5128",x,-.4,-1));dock.add(cyl(.1,.12,1.4,"#7B5128",x,-.4,1))});const hp=id==="winter"?P(15,-5):P(15,-8);dock.position.set(hp[0],0,hp[1]);mergeStatic(dock);scene.add(dock);
  ...
  if(ex("balloon")){const bal=new T.Group();bal.add(sph(1.8,"#FF8C1A",0,3.4,0,10));const st2=sph(1.82,"#FFBF00",0,3.4,0,10);st2.scale.set(.5,1,1);bal.add(st2);bal.add(box(.9,.7,.9,"#A9743F",0,0,0));[-.4,.4].forEach(x=>bal.add(cyl(.03,.03,2.3,"#1F2937",x,1.2,0,3)));const bp=P(-15,11);mergeStatic(bal);bal.position.set(bp[0],7,bp[1]);scene.add(bal);props.balloon=bal}
  ...
  if(ex("snowmen")){[[6,-4],[-7,9]].map(a=>P(a[0],a[1])).forEach(([x,z])=>{const g=new T.Group();g.add(sph(.7,"#F8FAFC",0,.6,0,9));g.add(sph(.5,"#F8FAFC",0,1.55,0,9));g.add(sph(.36,"#F8FAFC",0,2.25,0,9));g.add(cone(.1,.4,"#F97316",0,2.25,.5,5).rotateX(Math.PI/2)?cone(.1,.4,"#F97316",0,2.25,.5,5):null);g.add(box(.6,.1,.6,"#1F2937",0,2.55,0));g.add(box(.4,.35,.4,"#1F2937",0,2.75,0));g.position.set(x,0,z);mergeStatic(g);scene.add(g);obstacles.push([x,z,.9]);blobAdd(x,z,1)})}
...
function addPlaque(m){const g=new T.Group();
  ...
  g.position.set(m.pos[0]+1.7,0,m.pos[1]+1.3);g.rotation.y=-.3;g.traverse(o=>{if(o.isMesh)o.castShadow=true});mergeStatic(g);
