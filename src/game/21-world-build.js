...
function buildWorld(id){
  ...
  dirL=new T.DirectionalLight("#fff5d6",1.1);dirL.position.set(14,24,10);dirL.castShadow=true;dirL.shadow.mapSize.set(2048,2048);Object.assign(dirL.shadow.camera,{left:-28,right:28,top:28,bottom:-28,near:1,far:80});scene.add(dirL);
  ...
  if(ex("stall")){const stall=new T.Group();stall.add(box(2,.9,1,"#A9743F",0,.45,0));stall.add(box(2.4,.1,1.4,"#E11D48",0,2,0));[-1,1].forEach(x=>stall.add(cyl(.05,.05,2,"#7B5128",x,1,.5,4)));[["#FBBF24",-.6],["#22C55E",0],["#F87171",.6]].forEach(([c,x])=>stall.add(sph(.22,c,x,1.05,.2,6)));stall.position.set(4.5,0,2);stall.rotation.y=-.6;scene.add(stall);obstacles.push([4.5,2,1.3])}
  ...
  if(ex("airstrip")){const p1=plane("#F59E0B");p1.position.set(1.5,0,-16.5);scene.add(p1);props.plane=p1}
  const p2=plane(id==="prod"?"#7F1D1D":"#EF4444");scene.add(p2);props.plane2=p2;
  if(ex("balloon")){const bal=new T.Group();bal.add(sph(1.8,"#F472B6",0,3.4,0,10));const st2=sph(1.82,"#FBBF24",0,3.4,0,10);st2.scale.set(.5,1,1);bal.add(st2);bal.add(box(.9,.7,.9,"#A9743F",0,0,0));[-.4,.4].forEach(x=>bal.add(cyl(.03,.03,2.3,"#1F2937",x,1.2,0,3)));bal.position.set(-15,7,11);scene.add(bal);props.balloon=bal}
  ...
  props.mentors=[];MENTORS.filter(m=>m.world===id).forEach(m=>{const c=character({kind:"mentor",body:m.look.shirt,legs:"#374151",arms:"#F5D7BC",label:m.name,look:m.look});c.g.position.set(m.pos[0],0,m.pos[1]);c.g.rotation.y=Math.PI;scene.add(c.g);c.id=m.id;props.mentors.push(c);obstacles.push([m.pos[0],m.pos[1],.6]);
    const ring=new T.Mesh(new T.TorusGeometry(1.1,.05,6,24),new T.MeshBasicMaterial({color:S.path[m.id]==="deep"?"#22D3EE":S.path[m.id]==="skip"?"#FB7185":"#A78BFA",transparent:true,opacity:.6}));ring.rotation.x=Math.PI/2;ring.position.set(m.pos[0],.05,m.pos[1]);scene.add(ring);c.ring=ring});
  // shadow blob + marker
  ...
  marker=new T.Mesh(new T.RingGeometry(.3,.45,20),new T.MeshBasicMaterial({color:"#22D3EE",transparent:true,opacity:0,side:T.DoubleSide}));marker.rotation.x=-Math.PI/2;marker.position.y=.06;scene.add(marker);
...
function placeBuilding(k,pop){
  ...
  if(pop){g.scale.set(.01,.01,.01);g.userData.popT=0;for(let i=0;i<40;i++){const m=new T.Mesh(new T.BoxGeometry(.18,.18,.18),new T.MeshBasicMaterial({color:["#22D3EE","#8B5CF6","#FBBF24","#34D399"][i%4]}));m.position.copy(g.position).add(new T.Vector3((Math.random()-.5)*2,1,(Math.random()-.5)*2));m.userData.v=new T.Vector3((Math.random()-.5)*6,4+Math.random()*5,(Math.random()-.5)*6);m.userData.life=1.4+Math.random();scene.add(m);parts.push(m)}}
