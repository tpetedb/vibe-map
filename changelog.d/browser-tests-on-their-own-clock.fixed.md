- A browser test that takes a page fixture no longer depends on when or where
  ...
  green run. A test that wants a size or a browser the fixtures do not offer,
  or that hands a bare page to one of our own tools, opens the same helper
  instead of a context of its own, so the three rules reach the artifact
  pictures and the media tool's tests too. Two files still read a toast off
  the screen and two still build their own page; both lists are named in
  `tests/test_harness_determinism.py`, both may only shrink, and both are on
  issue 148.
