...
**Try in five minutes.** Write test_cart.py with a yield fixture cart that yields an empty list and clears it after the yield, one test that asks for cart, and one test parametrized over three (price, qty, total) rows. Run uv run --with pytest pytest -q test_cart.py > pytest-out.txt and read the last line: 4 passed.
