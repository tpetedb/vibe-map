...
| a topic of the tech tree: one TOML file under `vibemap/data/topics/<pack>/`, scaffolded by `tools/new_topic.py` (`vibemap/tech.py` is only the shape the generators read) | `just tree` (regenerates the notes, the tree JS, `docs/ROADMAP.md`), `just build` | both of the above |
...
Four things keep it fast. A `concurrency` group per ref cancels a superseded pull-request run and hands its runners back, and never cancels `main` or a tag, whose result is the record of that commit. `push` is limited to `main` and to tags so a commit on a branch with a pull request is tested once, not twice. `astral-sh/setup-uv` caches the resolved environment and `actions/cache` keeps `~/.cache/ms-playwright` under a key that is the resolved Playwright version, so a browser download happens on a lock-file bump and not otherwise; `playwright install --with-deps` still runs on a cache hit, because the system libraries WebKit needs on Linux live outside that folder.

The fourth is the shape of the browser battery. A Linux runner has no GPU, so the game renders through SwiftShader and one runner needs about twenty-five minutes for the Chromium half alone. Five runners share it instead: the `browser-shard` matrix runs `smoke`, `ui`, `bridges`, `panels` and `webkit` in parallel, cut along the per-file durations a run reported rather than a guess, and the job named `Playwright tests (Chromium and WebKit)` is a separate aggregator that needs them. Every shard prints `--durations`, which is how the split is rebalanced when it drifts. That separation is deliberate. A matrix job's name carries the leg in it, so it can never be the required context; and the aggregator runs under `if: always()`, because a required check on a job that is skipped stays pending and blocks the merge for ever. It asserts twice, since a required check that can go green while a shard did not run is worse than none: the aggregate result GitHub reports for the matrix job, and a receipt artifact from every shard the matrix defines. `tools/ci_shards.py` owns both halves and reads the split back out of the workflow, so the matrix stays the one place it is written down, and `tests/test_repo.py` fails when a browser test file is in no shard or in two.
...
- `CHANGELOG.md`: Keep a Changelog form, written only by a release from the fragments in `changelog.d/` (see [Catching up with main](#catching-up-with-main-and-the-files-that-conflict)).
