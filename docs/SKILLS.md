...
- Two skills pre-approve their own commands with `allowed-tools` (`grimoire-progress`: `uv run grimoire`, `just`; `duckdb-sql`: `duckdb`), so the learner is not asked for permission on every call.
...
Start `claude` in this folder. Type `/` and confirm all nine names appear. Then type each phrase in a fresh session and check the transcript for the `Skill(<name>)` call before the answer. One negative per skill: a phrase that must not load it.
