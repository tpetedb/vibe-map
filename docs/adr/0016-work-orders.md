...
Status: Accepted, 2026-09-23 (proposed 2026-09-21 in #144; every order since has been built, reviewed and landed this way)
...
**One tool decides, and everything calls it.** `tools/work.py` (standard library only, so a hook can run it with a bare `python3`) checks that orders are readable, that no two active orders own the same file, that a branch changed nothing outside its order, that every check passes, that the review is by someone else and rules on every criterion with evidence, and that every cross team signed. A review names the full commit id it read. It stays current only while neither the order file nor the branch's own contribution to the files it owns has changed since that commit. That contribution is compared as a verbatim patch id (`git patch-id --verbatim`): a clean sync with main keeps a review, and any edit ends it, a change of indentation included. The review and the sign-offs themselves are left out of the comparison, or an order that owns `work/` would end its own review by committing it. The `work-*` recipes, the hooks and CI are three doors to the same checks.
...
**Status is derived, never stored.** An order is active while its branch is checked out in some worktree and landed once its accepting review is on `main`. `just work-plan <goal>` turns a goal into launch groups: what an order needs comes first, no two orders in a group own the same file, and a group fits the budget of four. Landed orders are swept at a release, like changelog fragments.
...
- This very change was sent back by its own reviewer more than once. The first review found sixteen defects, three of them high: a test the change broke and CI hid, a scaffold that locked the edit hook, and a stop hook reading a field a subagent's report never reaches. The second found that an order owning `work/` ended its own review by committing it, that two tests passed only because the builder's machine ignores `__pycache__` globally, and that a change of indentation slipped past the review. The third found the mirror image: a test that passed on the builder's machine and failed on a runner, because the runner names its branch in the environment. Each fix has a test, and the tests now run with neither the machine's git configuration nor the runner's variables. That is the argument for the review stage.
...
- `touched.json` is the hook's memory of which agent edited under an order, and as what (builder, or reviewer when it only wrote into the order's folder). It is not committed. It keeps the 32 agents written most recently (`TOUCHED_KEEP` in `tools/work.py`) and drops the rest each time it is written, because four agents work at once and an entry that far behind belongs to an agent that stopped long ago. A missing entry costs an agent a reminder, never a check. The file goes with its order's folder when `just release` sweeps the landed orders.
- Known and accepted: agents on a Python older than 3.11 run without the hooks, with a line on stderr saying so.
